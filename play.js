...
// ============================================================
// IMPORT AUTOMATICO AUDIO GIÀ PRESENTI IN FIREBASE STORAGE
// ============================================================

let storageAudioCandidates = [];

async function loadStorageAudioFiles() {
  const box = document.getElementById("storageAudioBox");
  if (!box) return;

  box.innerHTML = `<p style="color: var(--text-secondary);">Controllo file audio in Firebase Storage...</p>`;
  ...
  try {
    ...
    songsSnap.docs.forEach(d => {
      const data = d.data();
      if (data.storageAudioPath) existingStoragePaths.add(data.storageAudioPath);
      if (data.audioUrl) existingAudioUrls.add(data.audioUrl);
    });

    const audioFolderRef = ref(storage, "audio");
    const result = await listAll(audioFolderRef);

    const candidates = [];
    for (const itemRef of result.items) {
      const fullPath = itemRef.fullPath;
      if (existingStoragePaths.has(fullPath)) continue;

      const audioUrl = await getDownloadURL(itemRef);
      if (existingAudioUrls.has(audioUrl)) continue;

      candidates.push({
        storageAudioPath: fullPath,
        fileName: itemRef.name,
        audioUrl,
        title: titleFromFileName(itemRef.name),
        artist: "CRAZY_MUSIC"
      });
    }

    storageAudioCandidates = candidates;
    renderStorageAudioCandidates();
  } catch (err) {
    box.innerHTML = `<div class="alert alert-error">Errore lettura Firebase Storage: ${escapeHtml(err.message)}</div>`;
  }
}

function renderStorageAudioCandidates() {
  const box = document.getElementById("storageAudioBox");
  if (!box) return;

  if (storageAudioCandidates.length === 0) {
    box.innerHTML = `<p style="color: var(--text-secondary);">Nessun nuovo audio trovato in Firebase Storage/audio.</p>`;
    return;
  }

  box.innerHTML = `
    <div class="alert alert-success">
      Trovati ${storageAudioCandidates.length} audio in Firebase Storage non ancora presenti nel pool.
    </div>
    <div style="display: grid; gap: 0.6rem; margin: 1rem 0;">
      ${storageAudioCandidates.map((s, i) => `
        <div class="song-item" style="grid-template-columns: 60px 1fr auto;">
          <img class="song-item-cover" src="${getFallbackCover(s.storageAudioPath)}" alt="">
          <div class="song-item-info">
            <input class="storage-title-input" data-index="${i}" data-field="title" value="${escapeAttr(s.title)}">
            <input class="storage-artist-input" data-index="${i}" data-field="artist" value="${escapeAttr(s.artist)}">
            <div style="color: var(--text-muted); font-size: 0.78rem; overflow-wrap: anywhere;">${escapeHtml(s.fileName)}</div>
          </div>
          <audio controls preload="none" style="height: 32px;" src="${s.audioUrl}"></audio>
        </div>
      `).join("")}
    </div>
    <button class="btn btn-primary" id="importStorageAudioBtn">Importa questi audio nel pool</button>
  `;

  box.querySelectorAll("input[data-index]").forEach(input => {
    input.addEventListener("input", () => {
      const idx = Number(input.dataset.index);
      const field = input.dataset.field;
      storageAudioCandidates[idx][field] = input.value.trim();
    });
  });

  document.getElementById("importStorageAudioBtn").addEventListener("click", importStorageAudioCandidates);
}

async function importStorageAudioCandidates() {
  const btn = document.getElementById("importStorageAudioBtn");
  const box = document.getElementById("storageAudioBox");
  btn.disabled = true;
  ...
  try {
    let imported = 0;
    for (const item of storageAudioCandidates) {
      const songId = `song_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
      await setDoc(doc(db, "songs", songId), {
        id: songId,
        title: item.title || titleFromFileName(item.fileName),
        artist: item.artist || "CRAZY_MUSIC",
        coverUrl: getFallbackCover(songId),
        audioUrl: item.audioUrl,
        storageAudioPath: item.storageAudioPath,
        storageCoverPath: "",
        createdAt: serverTimestamp()
      });
      imported++;
    ...
    box.innerHTML = `<div class="alert alert-success">Importati ${imported} audio nel pool. Ora sono disponibili per il torneo.</div>`;
    storageAudioCandidates = [];
    await loadSongs();
  } catch (err) {
    box.innerHTML = `<div class="alert alert-error">Errore importazione: ${escapeHtml(err.message)}</div>`;
  }
}

document.getElementById("refreshStorageAudioBtn")?.addEventListener("click", loadStorageAudioFiles);
